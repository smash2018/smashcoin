...
// Copyright (c) 2015 The HoldCoin developers
// Copyright (c) 2016 The SmashCoin developers
...
int nCoinbaseMaturity = COINBASE_MATURITY_SMASH;
...
const string strMessageMagic = "SmashCoin Signed Message:\n";
...
void static SyncWithWallets(const CTransaction& tx, const CBlock* pblock = NULL, bool fUpdate = false, bool fConnect = true)
{
    if (!fConnect)
    {
        // smashcoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // smashcoin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // smashcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// smashcoin: find block wanted by given orphan block
...
int64 GetProofOfWorkReward(int64 nFees)
{
  ...
  if(pindexBest != NULL)
    {
      height = pindexBest->nHeight + 1;
    }
  int64 nSubsidy = 0;
  do{
    // 20160920 PCW Mod
    if (height == 10) // premine
      {
    nSubsidy = 4000000000000;
    if (fDebug && GetBoolArg("-printcreation"))
	  printf("GetProofOfWorkReward() : create=%s nSubsidy=%"PRI64d" nFees=%"PRI64d"\n", FormatMoney(nSubsidy).c_str(), nSubsidy, nFees);
	break;
      }
    if(height < BLOCKS_CONSTANT_END_OF_POW)  // 5250000 blocks (the end of POW)
      {
    nSubsidy = 15 * COIN;  // initial
    int64 nStep = height / BLOCKS_CONSTANT_OF_REWARDS_W; // per 105000 blocks
	for(int i=0; i<nStep; i++)
	  {
        if(i<14)
          {
            nSubsidy = nSubsidy * 90 / 100;  // interest rate
          } else {
            nSubsidy = nSubsidy * 105 / 100;  // interest rate
          }
	  }
	if (fDebug && GetBoolArg("-printcreation"))
...
// smashcoin: miner's coin stake is rewarded based on coin age spent (coin-days)
int64 GetProofOfStakeReward(int64 nCoinAge, int64 nFees)
{
  int64 nRewardCoinYear = MAX_MINT_PROOF_OF_STAKE;  // initial
  int height = 0;
  if(pindexBest != NULL)
    {
      height = pindexBest->nHeight + 1;
  ...
  // 20160920 PCW Mod
  int64 nStep = height / BLOCKS_CONSTANT_OF_REWARDS_S; // per 105000 blocks
  if( nStep <= ((MAX_MINT_PROOF_OF_STAKE - MIN_MINT_PROOF_OF_STAKE) / (0.006 * COIN) ))
    {
      nRewardCoinYear = MAX_MINT_PROOF_OF_STAKE - nStep * 0.006 * COIN;
    }
  else
    {
      nRewardCoinYear = MIN_MINT_PROOF_OF_STAKE;
    }
...
static const int64 nTargetSpacingWorkMax = 16 * STAKE_TARGET_SPACING; // 2-hour
...
// smashcoin: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // smashcoin: target change every block
    // smashcoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // smashcoin: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // smashcoin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // smashcoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // smashcoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // smashcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
  ...
  // smashcoin: track money supply and mint amount info
...
// smashcoin: total coin age spent in transaction, in the unit of coin-days.
...
// smashcoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // smashcoin: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // smashcoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // smashcoin: record proof-of-stake hash value
    ...
    // smashcoin: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // smashcoin: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // smashcoin: coinbase output should be empty if proof-of-stake block
    ...
    // smashcoin: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // smashcoin: check that the block satisfies synchronized checkpoint
    ...
    // smashcoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // smashcoin: check proof-of-stake
    ...
    // smashcoin: verify hash target and signature of coinstake tx
    ...
    // smashcoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // smashcoin: getblocks may not obtain the ancestor block rejected
    ...
    // smashcoin: if responsible for sync-checkpoint send it
...
// smashcoin: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "SmashCoin", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "SmashCoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    //if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "10-01-2016 SmashCoin";
        CTransaction txNew;
        txNew.nTime = 1475290800; // 10-01-201 12:00:00
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1475290800;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1256526664u;

        if (fTestNet)
        {
            block.nTime    = 1446535713;
            block.nNonce   =457331u;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xc7f4211115afcda48f6109067c157c1276d5abe9897182b4d9cf8ce570545034"));
    if(false && (block.GetHash() != hashGenesisBlockOfficial))
        //if(true && (block.GetHash() != hashGenesisBlockTestNet)) // for testnet
	  {
        ...
        // smashcoin: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // smashcoin: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // smashcoin: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // smashcoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // smashcoin: if detected invalid checkpoint enter safe mode
    ...
    // smashcoin: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // smashcoin: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // smashcoin: record my external IP reported by peer
        ...
        // smashcoin: relay sync-checkpoint
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // smashcoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // smashcoin: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // smashcoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // smashcoin: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // smashcoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
