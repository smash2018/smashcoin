...
{
    // Wait for IRC to get it first - disabled with smashcoin
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "SmashCoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
// testnet dns seed begins with 't', all else are smashcoin dns seeds.
static const char *strDNSSeed[][2] = {
   // {"seed", "seed.smashcoin.net"},
   // {"seedppc", "seedppc.smashcoin.net"},
   // {"7server", "ppcseed.ns.7server.net"},
   // {"altcointech", "dnsseed.ppc.altcointech.net"},
   // {"diandianbi", "seed.diandianbi.org"},
   // {"tnseed", "tnseed.smashcoin.net"},
   // {"tnseedppc", "tnseedppc.smashcoin.net"},
};
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    if (true /*!fTestNet*/)  // smashcoin enables dns seeding with testnet too
    {
...
unsigned int pnSeed[] =
{
    0x559ec634, 0xdcaba023, 0xdc3e3234
};
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  SmashCoin is probably already running."), ntohs(sockaddr.sin_port));
        else
...
void StartNode(void* parg)
{
    ...
    // smashcoin: mint proof-of-stake blocks in the background
