...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx)
{
...

    {
                                        ...
                                        if(txdb.ReadDiskTx(prevout.hash, prev))
                                        {
                                            if (prevout.n < prev.vout.size())
                                            {
                                                ...
                                                if (ExtractAddress(vout.scriptPubKey, address))
                                                {
                                                    ...
                                                    strHTML += tr("<b>From:</b> ") + QString::fromStdString(address.ToString()) + "<br>";
                                                }
                                            }
                                        }
                                    }
        ...
        if (wtx.IsCoinBase() && nCredit == 0)
        {
            ...
            if (wtx.IsInMainChain())
                strHTML += tr("(%1 matures in %2 more blocks)")
                        .arg(BitcoinUnits::formatWithUnit(BitcoinUnits::smash, nUnmatured))
        ...
        else if (nNet > 0)
        {
            ...
            // Credit
            //
            strHTML += tr("<b>Credit:</b> ") + BitcoinUnits::formatWithUnit(BitcoinUnits::smash, nNet) + "<br>";
        }
        else
        {
            ...
            if (fAllFromMe)
            {
                ...
                BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                {
                    ...
                    strHTML += tr("<b>Debit:</b> ") + BitcoinUnits::formatWithUnit(BitcoinUnits::smash, -txout.nValue) + "<br>";
                }
                ...
                if (fAllToMe)
                {
                    ...
                    strHTML += tr("<b>Debit:</b> ") + BitcoinUnits::formatWithUnit(BitcoinUnits::smash, -nValue) + "<br>";
                    strHTML += tr("<b>Credit:</b> ") + BitcoinUnits::formatWithUnit(BitcoinUnits::smash, nValue) + "<br>";
                }

                int64 nTxFee = nDebit - wtx.GetValueOut();
                if (nTxFee > 0)
                    strHTML += tr("<b>Transaction fee:</b> ") + BitcoinUnits::formatWithUnit(BitcoinUnits::smash,-nTxFee) + "<br>";
            }
            else
            {
                //
                ...
                BOOST_FOREACH(const CTxIn& txin, wtx.vin)
                    if (wallet->IsMine(txin))
                        strHTML += tr("<b>Debit:</b> ") + BitcoinUnits::formatWithUnit(BitcoinUnits::smash,-wallet->GetDebit(txin)) + "<br>";
                BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                    if (wallet->IsMine(txout))
                        strHTML += tr("<b>Credit:</b> ") + BitcoinUnits::formatWithUnit(BitcoinUnits::smash,wallet->GetCredit(txout)) + "<br>";
            }
        }

        strHTML += tr("<b>Net amount:</b> ") + BitcoinUnits::formatWithUnit(BitcoinUnits::smash,nNet, true) + "<br>";
        ...
        if (fDebug)
        {
            ...
            BOOST_FOREACH(const CTxIn& txin, wtx.vin)
                if(wallet->IsMine(txin))
                    strHTML += "<b>Debit:</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::smash,-wallet->GetDebit(txin)) + "<br>";
            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                if(wallet->IsMine(txout))
                    strHTML += "<b>Credit:</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::smash,wallet->GetCredit(txout)) + "<br>";
                ...
                BOOST_FOREACH(const CTxIn& txin, wtx.vin)
                {
                    ...
                    if(txdb.ReadDiskTx(prevout.hash, prev))
                    {
                        if (prevout.n < prev.vout.size())
                        {
                            ...
                            strHTML = strHTML + " Amount=" + BitcoinUnits::formatWithUnit(BitcoinUnits::smash,vout.nValue);
